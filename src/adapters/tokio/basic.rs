//! The tokio basic-receiver adapter.
...
use crate::receiver::{BasicReceiver as Core, BasicReceiverEvent, ReceiverConfig};
...
/// An asynchronous sACN basic receiver driven by the tokio runtime.
///
/// Construct one with [`bind`](Self::bind), register universes with
/// [`listen`](Self::listen) / [`listen_on`](Self::listen_on), and consume
/// per-source notifications with [`next_event`](Self::next_event). For merged
/// universe data, use the merging [`Receiver`](super::Receiver) instead.
///
...
/// use sacn::{BasicReceiverEvent, ReceiverConfig, Universe};
///
/// # async fn demo() -> Result<(), sacn::AdapterError> {
/// let mut rx = BasicReceiver::bind(ReceiverConfig::new()).await?;
...
///             let _ = data.values;
///         }
///         BasicReceiverEvent::SourcesLost { universe, .. } => { let _ = universe; }
///         _ => {}
///     }
/// }
/// # Ok(())
...
pub struct BasicReceiver {
    ...
    recv_buf: Vec<u8>,
    /// Events produced by methods that have not yet been handed to the caller.
...
impl BasicReceiver {
    ...
    /// Binds a receiver to an explicit local address. Shared by [`bind`] and the
    /// tests, which bind an ephemeral port to stay isolated from one another.
    ///
    /// [`bind`]: Self::bind
    async fn bind_to(addr: SocketAddr, config: ReceiverConfig) -> Result<Self, AdapterError> {
        Ok(Self {
    ...
    /// sampling period (a [`SamplingStarted`](BasicReceiverEvent::SamplingStarted)
    ...
    /// Waits for and returns the next [`BasicReceiverEvent`].
    ///
    ...
    /// and returns events as they are produced. Malformed packets are dropped
    /// silently.
    ///
                ...
                result = self.socket.recv_from(&mut self.recv_buf) => {
                    if let Ok((len, from)) = result {
                        ...
                        if let Ok(packet) = Packet::parse(&self.recv_buf[..len]) {
                            ...
                            // The receiving interface is not yet attributed
                            // (would require per-packet pktinfo), so pass UNKNOWN.
                            let outcome =
